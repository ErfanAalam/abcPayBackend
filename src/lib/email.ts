...
export async function sendOtpEmail(to: string, otp: string) {
  await transporter.sendMail({
    from: `"Crypto Profit Zone" <${process.env.SMTP_USER}>`,
    to,
    subject: "Verify your email - Crypto Profit Zone",
    html: `
      <div style="font-family: sans-serif; padding: 20px; background: #0f172a; color: #fff; border-radius: 12px;">
        <h2 style="color: #6366f1;">Crypto Profit Zone Email Verification</h2>
